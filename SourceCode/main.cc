...
namespace PhaseField
{
  ...
  private:
    ...
    double line_search_stepsize_strong_wolfe(
        ...
        const BlockVector<double> &solution_delta, unsigned int &num_ls);

    double
    line_search_stepsize_gradient_based(const BlockVector<double> &BFGS_p_vector,
  ...
  void PhaseFieldMonolithicSolve<dim>::update_qph_incremental(
      const BlockVector<double> &solution_delta,
      const BlockVector<double> &solution_old, const bool is_print)
  {
  ...
  template <int dim>
  std::vector<double> PhaseFieldMonolithicSolve<dim>::solve_linear_system(
  ...
  template <int dim> void PhaseFieldMonolithicSolve<dim>::print_conv_header_BFGS()
  {
  ...
  template <int dim>
  bool PhaseFieldMonolithicSolve<dim>::solve_nonlinear_timestep_newton(
      BlockVector<double> &solution_delta)
  {
         ...
         ++newton_iteration)
    {
          ...
          m_error_residual_norm.m_d <= m_parameters.m_tol_d_residual)
      {
        ...
        m_logfile << "\t\tConvergence is reached after " << newton_iteration
      ...
      {
        m_logfile << " | " << std::fixed << std::setprecision(3) << std::setw(7)
                  << std::scientific << "  " << linear_solver_parameters[0]
  ...
  template <int dim>
  void PhaseFieldMonolithicSolve<dim>::solve_nonlinear_timestep_BFGS(
      BlockVector<double> &solution_delta)
  {
    AssertThrow(false,
    ...
    for (; BFGS_iteration < m_parameters.m_max_iterations_BFGS; ++BFGS_iteration)
    {
      ...
      line_search_parameter = line_search_stepsize_strong_wolfe(
          phi_0, phi_0_prime, BFGS_p_vector_block, solution_delta,
          num_line_search);
      ...
      if (BFGS_iteration == 1)
      {
        double scale_parameter =
            (BFGS_y_vector * BFGS_p_vector) / (BFGS_y_vector.norm_sqr());
      ...
      m_logfile << " | " << std::fixed << std::setprecision(3) << std::setw(7)
                << std::scientific << "  " << line_search_parameter << "       "
                << energy_functional << "  " << m_error_residual_norm.m_norm
                << "  " << m_error_residual_norm.m_u << "  "
                << m_error_residual_norm.m_d << "  " << m_error_update_norm.m_norm
         ...
         ++LBFGS_iteration)
    {
      ...
      */
      ...
      if (m_parameters.m_type_line_search == "StrongWolfe")
      {
        ...
        line_search_parameter = line_search_stepsize_strong_wolfe(
            phi_0, phi_0_prime, LBFGS_r_vector, solution_delta, num_line_search);
      }
      else if (m_parameters.m_type_line_search == "GradientBased")
      {
        ...
        line_search_parameter = line_search_stepsize_gradient_based(
            LBFGS_r_vector, solution_delta, num_line_search);
      }
      else
      {
      ...
      if (m_parameters.m_output_iteration_history)
      {
        ...
        m_logfile << " | " << std::fixed << std::setprecision(3) << std::setw(1)
                  << std::scientific << "" << line_search_parameter << "  "
                  << std::setw(3) << num_line_search << "  " << std::fixed
